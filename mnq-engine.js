...
function detectSetup(candles,options={}){
  ...
  const entry=roundTick(last.close),contracts=Math.max(1,Number(options.contracts)||1),stopDollar=Number(options.stopDollar)||Number(options.riskBudget)||100,targetDollar=Number(options.targetDollar)||stopDollar*1.3,riskPoints=stopDollar/(contracts*MNQ.pointValue),rewardPoints=targetDollar/(contracts*MNQ.pointValue),stop=roundTick(direction==='LONG'?entry-riskPoints:entry+riskPoints),target=roundTick(direction==='LONG'?entry+rewardPoints:entry-rewardPoints),actualRiskPoints=Math.abs(entry-stop),actualRewardPoints=Math.abs(target-entry);
