...
function detectSetup(candles,options={}){
  ...
  const entry=roundTick(last.close),stop=roundTick(direction==='LONG'?entry-a*1.3:entry+a*1.3),riskPoints=Math.abs(entry-stop),target=roundTick(direction==='LONG'?entry+riskPoints*1.3:entry-riskPoints*1.3);
  return {state:rejection?'ENTRADA_CONFIRMADA':'AGUARDANDO_PULLBACK',reason:rejection?'Pullback e rejeição confirmados.':'BOS confirmado; aguardando preço rejeitar a zona 50–61,8%.',direction,macro,atr:a,broken,impulse:{start,end},fib:{fifty:fib50,sixtyOneEight:fib618,zoneLow,zoneHigh},inZone,rejection,entry,stop,target,riskPoints,riskUsdPerContract:riskPoints*MNQ.pointValue,rewardUsdPerContract:Math.abs(target-entry)*MNQ.pointValue};
}
