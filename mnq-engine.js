...
function detectSetup(candles,options={}){
  ...
  const entry=roundTick(last.close),contracts=Math.max(1,Number(options.contracts)||1),stopDollar=Number(options.stopDollar)||0,targetDollar=Number(options.targetDollar)||0,atrRiskPoints=a*(Number(options.stopAtr)||1.3),riskPoints=stopDollar?stopDollar/(contracts*MNQ.pointValue):atrRiskPoints,rewardPoints=targetDollar?targetDollar/(contracts*MNQ.pointValue):riskPoints*(Number(options.rewardRisk)||1.3),stop=roundTick(direction==='LONG'?entry-riskPoints:entry+riskPoints),target=roundTick(direction==='LONG'?entry+rewardPoints:entry-rewardPoints),actualRiskPoints=Math.abs(entry-stop),actualRewardPoints=Math.abs(target-entry);
  return {state:rejection?'ENTRADA_CONFIRMADA':'AGUARDANDO_PULLBACK',reason:rejection?'Pullback e rejeição confirmados.':'BOS confirmado; aguardando preço rejeitar a zona 50–61,8%.',direction,macro,atr:a,broken,impulse:{start,end},fib:{fifty:fib50,sixtyOneEight:fib618,zoneLow,zoneHigh},inZone,rejection,entry,stop,target,riskPoints:actualRiskPoints,rewardPoints:actualRewardPoints,riskUsdPerContract:actualRiskPoints*MNQ.pointValue,rewardUsdPerContract:actualRewardPoints*MNQ.pointValue,riskUsdTotal:actualRiskPoints*MNQ.pointValue*contracts,rewardUsdTotal:actualRewardPoints*MNQ.pointValue*contracts,contractsRequested:contracts,stopMode:stopDollar?'DOLLAR':'ATR',targetMode:targetDollar?'DOLLAR':'RR'};
}
