...
async function api(req, res, pathname) {
  ...
  try {
    ...
    if (pathname === '/api/mnq/analyze' && req.method === 'POST') {
      ...
      const blockers=[];if(risk.blocked)blockers.push('Limite de perda da conta atingido.');if(risk.warning)blockers.push('Menos de 20% do drawdown disponível.');if(setup.state!=='ENTRADA_CONFIRMADA')blockers.push(setup.reason);if(setup.state==='ENTRADA_CONFIRMADA'&&!contracts)blockers.push('Risco disponível não comporta 1 MNQ.');
