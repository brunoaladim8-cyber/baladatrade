...
async function api(req, res, pathname) {
  ...
  try {
    ...
    if (pathname === '/api/mnq/analyze' && req.method === 'POST') {
      const data=await body(req),candles=Array.isArray(data.candles)&&data.candles.length?data.candles:await mnqCandles(),profile=String(data.profile||'tpt_test'),accountSize=Number(data.accountSize||50000),riskBudget=Number(data.riskBudget||100),requestedContracts=Math.max(1,Math.floor(Number(data.contracts)||1)),stopDollar=Math.max(0,Number(data.stopDollar)||0),targetDollar=Math.max(0,Number(data.targetDollar)||0);
    ...
    if (pathname === '/api/positions/watch' && req.method === 'POST') {const data=await body(req),symbol=String(data.symbol||'').toUpperCase(),direction=String(data.direction||'LONG').toUpperCase(),entry=Number(data.entry),quantity=Number(data.quantity),stop=Number(data.stop||0),target=Number(data.target||0),trailingPct=Number(data.trailingPct||0);if(!/^[A-Z0-9]{5,20}$/.test(symbol)||!['LONG','SHORT'].includes(direction)||!Number.isFinite(entry)||entry<=0||!Number.isFinite(quantity)||quantity<=0||trailingPct<0||trailingPct>20)return json(res,400,{error:'Monitor inválido.'});const saved=await savePositionWatch({symbol,direction,entry,quantity,stop,target,trailingPct});return json(res,201,{ok:true,saved,monitor:await monitorPositions()});}
